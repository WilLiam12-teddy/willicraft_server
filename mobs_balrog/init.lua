...
local CHANCE = tonumber(minetest.settings:get("mobs_balrog_chance")) -- CHANCES DE APARECER ( CASO DEMORE , TENTAR: 8000)
if (CHANCE == nil) then
	CHANCE = 5000
end
...
local MIN_HEIGHT = tonumber(minetest.settings:get("mobs_balrog_min_height")) -- MMINIMO DE ALTURA
if (MIN_HEIGHT == nil) then
	MIN_HEIGHT = -8500
end

local MAX_HEIGHT = tonumber(minetest.settings:get("mobs_balrog_max_height")) -- MAXIMODE LTURA
...
mobs:spawn({name = "mobs_balrog:balrog",
	nodes = {"shadow:balrog_spawn"},   -- BLOCO DE SPAWN 
	max_light = MAX_LIGHT,
	min_light = MIN_LIGHT,
	interval = INTERVAL,
	chance = CHANCE,
